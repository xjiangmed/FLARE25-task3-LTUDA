...
class model_train_data_process(object):
    ...
    def process(self, image_id):
        ...
        image, image_spacing, image_direction, image_itk_info = load_data(
            os.path.join(self.image_path, data_id + "_0000.nii.gz"))
        mask, _, mask_direction, label_itk_info = load_data(
            os.path.join(self.mask_path, data_id + ".nii.gz")
        )
        ...
        # #有些伪标签包含大于13的gt值，例如128, 129和137
        # min_ = np.min(mask)
        # max_ = np.max(mask)
        # gt_values = np.unique(mask)
        # num_classes = 14
        # if not (min_>=0 and max_< num_classes):
        #     mask[mask>13] = 0
