...
import wandb
from .dataloader import DataLoaderX, build_loader, build_loader_lnul
...
from .trainer_CDA import Trainer_CDA
...
class Training_CDA(object):
    ...
    def _main_worker(self, local_rank, config):
        ...
        if self.train_type == "fine":
            labeled_train_loader, unlabeled_train_loader, val_loader = build_loader_lnul(
                config,
        ...
        lr_scheduler = build_scheduler(config, optimizer, len(unlabeled_train_loader))
        trainer = Trainer_CDA(
            config=config,
